...
var _ = Describe("Smoke Tests", func() {
	...
	Context("Kubernetes deployment", func() {
		BeforeEach(func() {
			runCmd("kubectl", "apply", "-f", "../assets/ncat-deploy.yml")
		})

		It("emits logs for an app to an aggregate drain", func() {
			By("redeploying cf with an aggregate drain")
			cfk8slogging := os.Getenv("CF_FOR_K8s_LOGGING")
			cfinstallvalues := os.Getenv("CF_INSTALL_VALUES")

			runCmd("ytt", "-f", cfk8slogging+"/config", "-f", "../assets/aggregate-drain.yml", "-f", cfinstallvalues)
